...
import com.adanac.commonclient.fastdfs_client.ClientGlobal;
import com.adanac.commonclient.fastdfs_client.ProtoCommon;
import com.adanac.commonclient.fastdfs_client.TrackerClient;
import com.adanac.commonclient.fastdfs_client.TrackerGroup;
import com.adanac.commonclient.fastdfs_client.TrackerServer;
import com.adanac.commonclient.imageserver.FileManagerPoolConfig;
...
public class ConnectionPool {
	...
	private String tgStr = FileManagerPoolConfig.connnectString;
	// the server port
	private int port = FileManagerPoolConfig.port;
	// the limit of connection instance
	private int size = FileManagerPoolConfig.size;
	...
	public TrackerServer checkout(int waitTimes) throws InterruptedException {
		...
		if (client1 == null) {
			ImageServerPoolSysout.warn("ImageServerPool wait time out ,return null");
	...
	public synchronized void drop(TrackerServer trackerServer) {
		...
		if (busyConnectionPool.remove(trackerServer) != null) {
			try {
				ImageServerPoolSysout.warn("ImageServerPool drop a connnection");
				ImageServerPoolSysout
						.warn("ImageServerPool size:" + (busyConnectionPool.size() + idleConnectionPool.size()));
				TrackerClient trackerClient = new TrackerClient();
				trackerServer = trackerClient.getConnection();
			} catch (IOException e) {
				trackerServer = null;
				ImageServerPoolSysout.warn("ImageServerPool getConnection generate exception");
				e.printStackTrace();
			} finally {
	...
	private void detector() {
		...
		new Thread() {
			...
			public void run() {
				...
				// ImageServerPoolSysout.warn("busyConnectionPool start close
				// trackerserver");
				// for(Entry<TrackerServer, Object>
				// entry:busyConnectionPool.entrySet()){
				// try {
				// entry.getKey().close();
				// } catch (Exception e) {
				// e.printStackTrace();
